...
import { Scales, Ruler, GenderIntersex, GenderMale, GenderFemale } from "@phosphor-icons/react";
...
// Positions fixes dans le viewport
const ECHO_PHOTOS = [
  {
    src: echoSecret,
...
function PolaroidCard({ photo }) {
  ...
  // Le wrapper est positionné exactement comme la photo.
  // Il englobe photo + bulle, overflow:visible pour que la bulle dépasse.
  // pointerEvents:auto pour réactiver les events sur ce seul élément.
  return (
    <div
      ...
      style={{
        ...
        transform: hovered
          ...
          : `rotate(${photo.rotation}) scale(1)`,
        transition: "transform 0.4s cubic-bezier(.34,1.56,.64,1), filter 0.3s",
        filter: hovered
          ? "drop-shadow(0 20px 40px rgba(44,36,32,0.3))"
          : "drop-shadow(0 6px 16px rgba(44,36,32,0.18))",
        opacity: 0.95,
      }}
    >
      {/* Polaroid */}
      ...
      {/* Bulle — absolute dans le wrapper, dépasse grâce à overflow:visible */}
      <div style={{
        position: "absolute",
        top: "20%",
        ...
        boxShadow: "0 8px 24px rgba(44,36,32,.25)",
        zIndex: 30,
...
<span style={{display:"flex",alignItems:"center",gap:4}}>{b.sexe === "garcon" ? <GenderMale size={16} weight="duotone" color="#2980B9" /> : <GenderFemale size={16} weight="duotone" color="#C0392B" />}{b.sexe === "garcon" ? "Garçon" : "Fille"}</span>
...
function ResultsModal({ bets, results, onClose }) {
  ...
  // Taille : ±1cm, sinon le(s) plus proche(s)
  const diffsTaille = bets.map(b => Math.abs(parseInt(b.taille) - realTaille));
  const minDiffTaille = Math.min(...diffsTaille);
  const winsPoidsExactTaille = diffsTaille.filter(d => d <= 1).length > 0;
  const tailleWinners = winsPoidsExactTaille
  ...
  // Labels pour afficher si c'est exact ou "le plus proche"
  const poidsLabel  = winnersPoidsExact ? null : `(plus proche : ${(minDiffPoids * 1000).toFixed(0)}g d'écart)`;
  const tailleLabel = winsPoidsExactTaille ? null : `(plus proche : ${minDiffTaille}cm d'écart)`;
  const catLabels   = { sexe: null, poids: poidsLabel, taille: tailleLabel };
...
<span style={{display:"flex",alignItems:"center",gap:6}}>{results.sexe === "garcon" ? <GenderMale size={20} weight="duotone" color="#fff" /> : <GenderFemale size={20} weight="duotone" color="#fff" />}{results.sexe === "garcon" ? "Garçon" : "Fille"}</span>
            <span>{results.poids} kg</span>
            <span>{results.taille} cm</span>
          </div>
        </div>
        {[["sexe","Sexe"],["poids","Poids"],["taille","Taille"]].map(([cat, label]) => (
          <div key={cat} style={{ marginBottom:20,paddingBottom:20,borderBottom:"1px solid #EDE8E4" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
              <strong>{label}</strong>
<span style={{ display:"flex",alignItems:"center",gap:6,fontSize:"0.78rem",color:"#8B7B74" }}><img src={LOTS_IMG[cat]} alt="" style={{ width:32,height:32,objectFit:"contain",borderRadius:6 }} />{LOTS[cat]}</span>
            </div>
        ...
        <div>
          ...
          {ranked.map((b,i) => (
            <div key={b.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:"1px solid #F5F0EC",fontSize:"0.95rem" }}>
              ...
              <span>{b.prenom}</span>
              <span style={{ marginLeft:"auto",fontWeight:600,color:"#D4A843" }}>{score(b)}/3 pts</span>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const [currentBet, setCurrentBet]   = useState({ sexe: null, poids: "", taille: "" });
  const [showAll, setShowAll]         = useState(false);
  const [showResults, setShowResults] = useState(false);
  const [results, setResults]         = useState({ sexe: "fille", poids: "", taille: "" });
  ...
  const handleStart = () => {
    ...
    if (existingBet) setCurrentBet({ sexe: existingBet.sexe, poids: String(existingBet.poids), taille: String(existingBet.taille) });
    setStep("bets");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await upsertBet({ prenom: prenom.trim(), sexe: currentBet.sexe, poids: parseFloat(currentBet.poids), taille: parseInt(currentBet.taille) });
      ...
      <div style={{ position:"relative", zIndex:3, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", padding:"32px 16px 100px" }}>
              ...
              <div style={{ background:"rgba(255,255,255,0.92)", backdropFilter:"blur(8px)", borderRadius:20, padding:20, marginTop:8, boxShadow:"0 8px 32px rgba(44,36,32,.10)" }}>
                {!adminMode ? (
                  ...
                  <>
                    ...
                    <button onClick={() => results.poids && results.taille && setShowResults(true)}
                      disabled={!results.poids || !results.taille}
                      style={btnP({ background:"#D4A843", opacity: results.poids && results.taille ? 1 : 0.5 })}>
                      🏆 Voir les gagnants
                    </button>
                  </>
                )}
        ...
        {step === "bets" && (
          <>
            <div className="fadeUp" style={{ width:"100%", maxWidth:480, marginBottom:24 }}>
              ...
              <p style={{ color:"#8B7B74", fontSize:"0.9rem" }}>Remplissez les 3 critères pour valider</p>
            </div>
        ...
        {step === "confirm" && (
          <div className="fadeUp" style={{ background:"#fff", borderRadius:20, boxShadow:"0 8px 32px rgba(44,36,32,.10)", padding:32, width:"100%", maxWidth:480, textAlign:"center" }}>
            ...
            {[
              { label:"Sexe",   value: currentBet.sexe === "garcon" ? "Garçon" : "Fille", lot: LOTS.sexe },
              { label:"Poids",  value: `${currentBet.poids} kg`, lot: LOTS.poids },
              { label:"Taille", value: `${currentBet.taille} cm`, lot: LOTS.taille },
            ].map(row => (
              <div key={row.label} style={{ padding:"14px 0", borderBottom:"1px solid #EDE8E4", textAlign:"left" }}>
                ...
                <div style={{ fontSize:"0.75rem", color:"#D4A843", display:"flex", alignItems:"center", gap:5 }}><img src={LOTS_IMG[["sexe","poids","taille"].find(k => LOTS[k] === row.lot)]} alt="" style={{ width:16, height:16, objectFit:"contain" }} />{row.lot}</div>
              </div>
            ))}
        ...
        {step === "done" && (
          <div className="fadeUp" style={{ textAlign:"center", padding:20 }}>
            ...
            <button onClick={() => { setStep("name"); setPrenom(""); setCurrentBet({ sexe:null, poids:"", taille:"" }); }} style={btnG()}>
              ← Retour à l'accueil
            </button>
          </div>
        )}
